...
def yaml2svg(yaml_filename, width, height, matrix):
    svg = ''
    with open(yaml_filename) as stream:
        ...
        svg += '<g transform="matrix({0})" fill="none" stroke="#000000">'.format(matrix)

        docs = yaml.safe_load_all(stream)
        for doc in docs:
            path = catmull_rom_to_bezier(doc['data'])
            svg += '<path fill="none" stroke="#000000" stroke-linecap="round" stroke-width="1" d="{0}" />'.format(path)
...
if __name__ == '__main__':
    if len(sys.argv) != 5:
        ...
        print(yaml2svg(yaml_filename = sys.argv[1], width = sys.argv[2], height = sys.argv[3]), matrix = sys.argv[4])
